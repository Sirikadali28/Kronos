...
class AnomalyDetector:
    ...
    def detect_zscore(self, threshold=3):
        ...
        if std == 0:
            print(
                "[Z-Score] Warning: Standard deviation is zero. Skipping this method."
            )
            return []
    ...
    def detect_moving_average(self, window=7, std_multiplier=2):
        ...
        self.anomalies_ma = self.data[
            anomaly_mask & self.data.index.isin(valid_indices)
        ].index.tolist()
    ...
    def combine_results(self, voting_threshold=2):
        ...
        self.combined_anomalies = [
            idx for idx, vote_count in votes.items() if vote_count >= voting_threshold
        ]
    ...
    def print_summary(self):
        ...
        print(
            f"  Value Range: {self.data[self.column_name].min():.4f} to {self.data[self.column_name].max():.4f}"
        )
...
def generate_sample_data(n_points=100):
    ...
    print(
        f"  Generated {n_points} data points with {len(anomaly_indices)} synthetic anomalies"
    )
...
def main():
    ...
    try:
        ...
        print("  Output: anomaly-report.csv")
